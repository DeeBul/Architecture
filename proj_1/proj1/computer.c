...
int Execute ( DecodedInstr* d, RegVals* rVals) {
    ...
    if(d->op == 0) {
        ...
        switch(d->regs.r.funct) {
            ...
            case 0: {
                rVals->R_rd = rVals->R_rs << d->regs.r.shamt;
                return rVals->R_rd;
            }
            ...
            case 8: {
                // jr jumps to then return address so it does not use execute
                return d->regs.r.rs;
            }
        }
    }
    else if (d->op == 2 || d->op == 3) {
        //J format does not use execute because it jumps
        return d->regs.j.target;
    }
    else {
        unsigned int immediate = d->regs.i.addr_or_immed;
...
int Mem( DecodedInstr* d, int val, int *changedMem) {
    ...
    if(d->type == 0) {
        if (d->regs.r.funct == 35) {
            *changedMem = -1;
            val = val - 0x00400000; //since this is where we start in mips memory and we have to go down
            val = val / 4;
